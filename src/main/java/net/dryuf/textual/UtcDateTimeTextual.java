...
public class UtcDateTimeTextual extends DateTimeBaseTextual
{
	...
	public java.lang.Long		convert(String text, String style)
	{
		...
		if ((groups = StringUtil.matchText("^(\\d{1,2})\\s*\\.\\s*(\\d{1,2})\\s*\\.\\s*(\\d{4})\\s+(\\d{1,2}):(\\d{1,2}):(\\d{1,2})$", text)) != null) {
			...
			return java.lang.Long.parseLong(groups[1])*1000;
		}
		throw new RuntimeException("Invalid format for date, no check performed?");
	}

	@SuppressWarnings("cast")
	public String			format(java.lang.Long internal, String style)
	{
		return (new java.text.SimpleDateFormat("dd.MM.yyyy HH:mm:ss")).format(new java.util.Date(((java.lang.Long)internal).longValue()));
	}
	...
	{
		try {
			return java.util.Date.UTC(year-1900, month, day+1, hour, minute, second);
		}
		catch (Exception ex) {
			return null;
		}
	...
	{
		return getUiContext().localize(DateTimeTextual.class, "Format dd.mm.yyyy hh:mm:ss required");
	}
}
